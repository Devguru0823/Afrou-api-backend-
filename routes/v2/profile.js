...
let POST_MODEL = require('../../models/v2/post.js');
let PROFILE_MODEL = require('../../models/v2/profile.js');
let FRIEND_REQUEST_MODEL = require('../../models/v2/friend_request.js');
const MEDIA = require('../../models/v2/media');
const USER_PHOTO = require('../../models/v2/user_photos.js');
...
router.get('/:user_id/posts', function(req, res, next) {
    utility.mongoConnect(req, res, function (client) {
        let user_id = Number(req.params.user_id);
        POST_MODEL.getUserPosts(client, req, res, user_id, function (err, response) {
            client.close();
            if(err){
                next(err);
            }else{
