...
module.exports.getMyGroups = function (CLIENT, req, res, cb) {
	...
	groupCollection
		.find({ group_status: 'active', group_admins: req.authorization.user_id })
		.sort({ group_id: -1 })
...
module.exports.getMembershipMyGroups = function (CLIENT, req, res, cb) {
	...
	let groupCollection = CONNECTION.collection(Model.collection_name);
	groupCollection
		.find({ group_status: 'active', group_members: req.authorization.user_id })
		.sort({ group_id: -1 })
		.toArray((err, groupList) => {
			if (err) {
				cb(err);
		...
		function (err, validatedData) {
			if (err) {
				...
				groupCollection.findOneAndUpdate(
					{ group_id: group_id, group_admins: currentUser },
					{ $set: validatedData },
...
module.exports.deleteMyGroup = function (CLIENT, req, res, cb) {
	...
	groupCollection.deleteOne(
		{ group_id: group_id, group_admins: req.authorization.user_id },
		(err, deletedResponse) => {
			if (err) {
				cb(err);
			} else {
...
module.exports.leaveOthersGroup = function (CLIENT, req, res, cb) {
	...
	groupCollection.findOneAndUpdate(
		...
		{ $pull: { group_members: req.authorization.user_id } },
		(err, leaveData) => {
			if (err) {
				cb(err);
			} else {
		...
		function (isMember) {
			if (isMember) {
				groupCollection.findOneAndUpdate(
					{
						group_id: group_id,
						group_members: { $ne: user_id },
						group_invited_members: { $ne: user_id },
					},
					{ $push: { group_invited_members: user_id } },
					(err, groupDetailsUpdate) => {
						if (err) {
							cb(err);
						} else {
							if (groupDetailsUpdate.value) {
								...
								// Email
								userCollection
									.find({ user_id: { $in: [requested_by, requested_to] } })
									.toArray(async (err, usersData) => {
										// Send Email
										let fromUser = null;
										let toUser = null;
										...
										EMAIL_HANDLER.sendGroupInvitationEmail(
											fromUser,
											toUser,
											groupDetailsUpdate.value
										);
	...
	groupCollection.findOneAndUpdate(
		{
			group_id: group_id,
			group_admins: { $ne: req.authorization.user_id },
			group_members: { $ne: req.authorization.user_id },
			group_invited_members: req.authorization.user_id,
		},
		{
			$push: { group_members: req.authorization.user_id },
			$pull: { group_invited_members: req.authorization.user_id },
		},
		async (err, acceptedResponse) => {
			if (err) {
				cb(err);
			}
			if (acceptedResponse) {
...
module.exports.getGroupDetails = function (CLIENT, req, res, cb) {
	...
	groupCollection.findOne(
		{ group_id: group_id, group_status: 'active' },
		(err, groupDetails) => {
			if (err) {
				cb(err);
			} else {
				if (groupDetails) {
					if (
						groupDetails.group_members.indexOf(req.authorization.user_id) === -1
					) {
						// Not a member yet
						if (
							groupDetails.group_invited_members &&
							groupDetails.group_invited_members.indexOf(
								req.authorization.user_id
							) !== -1
						) {
							// Not invited Yet
					...
					// Admin
					groupDetails.admin =
						groupDetails.group_admins.indexOf(req.authorization.user_id) !== -1;

					const postCollection = CONNECTION.collection(
						POST_MODEL.collection_name
					);
		...
		if (err) {
			...
			if (groupDetails) {
				if (groupDetails.group_status == 'active') {
					var returnUsers = [];
					//   userCollection.find({ "user_id": { $in: groupDetails.group_members } }).toArray((err,doc)=>{console.log(err,doc)})
					userCollection
						.find({ user_id: { $in: groupDetails.group_members } })
							...
							if (err) {
								...
								await users.forEach((user) => {
									returnUsers.push({
										...
										admin:
											groupDetails.group_admins.indexOf(user.user_id) !== -1,
									});
								});
								let finalResponse = {};
								finalResponse.status = true;
...
module.exports.getFriendListForGroupInvitation = function (
	CLIENT,
	req,
	res,
	cb
	...
	const currentUser = req.authorization.user_id;
	groupCollection.findOne(
		{ group_id: group_id, group_status: 'active', group_members: currentUser },
		(err, groupDetails) => {
			if (err) {
				cb(err);
			} else {
											...
											users.forEach((user) => {
												...
												if (invitationSent.indexOf(user.user_id) === -1) {
													tmpUser.button = {
														...
														button_text: 'Invite',
														button_link:
															'/groups/' + group_id + '/invite/' + user.user_id,
													};
												} else {
													tmpUser.button = {
														button_type: 'sent',
...
module.exports.findGroups = function (CLIENT, req, res, cb) {
	...
	groupCollection
		.find({
			...
			$and: [
				{
					$or: [{ private: false }, { private: true, group_members: user_id }],
				},
				{
					$or: [
		...
		async (err, acceptedResponse) => {
			if (err) {
				...
				if (acceptedResponse.value) {
					...
					let notification_details = {
						text_template:
							'{{USER_ID_' + requested_by + '}} has Joined your Group',
						group_id: group_id,
					};
					let notification_type = 'group_invite_accept';
