	...
	utility.validatePostData(
		...
		function (err, validatedData) {
			if (err) {
				...
				postCommentCollection.insertOne(
					validatedData,
									...
									if (requested_to !== requested_by) {
										...
										if (resp.value.posted_for === 'group') {
											notification_details.text_template =
												'{{USER_ID_' +
												requested_by +
												'}} commented on your post in a Group';
										}
	...
	utility.validatePostData(
		...
		function (err, validatedData) {
			if (err) {
				...
				postCommentCollection.findOneAndUpdate(
					{ comment_id: comment_id, commented_by: req.authorization.user_id },
					{ $set: validatedData },
					function (err, response) {
						if (err) {
							cb(err);
...
module.exports.deletePostComment = function (CLIENT, req, res, cb) {
	...
	postCommentCollection.findOneAndDelete(
		{ comment_id: comment_id, commented_by: req.authorization.user_id },
		function (err, response) {
			if (err) {
				cb(err);
				...
				if (response?.value) {
					postCommentCollection.deleteMany(
						{ comment_parent_id: comment_id },
						function (err, deleted) {
							let totalDeletedCount = 1 + deleted.deletedCount;
							let postCollection = CONNECTION.collection(
								POST_MODEL.collection_name
							);
							postCollection.findOneAndUpdate(
								{ post_id: response.value.post_id },
								{ $inc: { comment_count: -totalDeletedCount } },
								{ returnOriginal: false },
								function (err, resp) {
									let finalResponse = {};
									finalResponse.status = true;
									finalResponse.data = {
										comment_count: resp.value.comment_count,
									};
									cb(null, finalResponse);
								}
							);
			...
			if (err) {
				...
				postCommentCollection.deleteMany(
					...
					function (err, deleted) {
						let totalDeletedCount = 1 + deleted?.deletedCount;
						storyCollection.findOneAndUpdate(
							{ story_id: response?.value?.story_id },
							{ $inc: { comment_count: -totalDeletedCount } },
							{ returnOriginal: false },
							function (err, resp) {
								let finalResponse = {};
							...
							postCollection.findOneAndUpdate(
								...
								async function (err, resp) {
									...
									if (requested_to !== requested_by) {
										...
										await userCollection
											.findOne({ user_id: Number(req.body.user_id) })
											.then((Details) => {
												if (!Details) {
													return cb('user_id  not found');
												}
												let pushData = {
													status: 'Post Comment',
													title: 'Post Comment',
												...
												NOTIFICATION.addNotification(
													...
													function (err, notificationResp) {
														let finalResponse = {};
														finalResponse.status = true;
														finalResponse.data = response.validatedData;
														cb(null, finalResponse);
													}
												);
											})
	...
	utility.validatePostData(
		...
		function (err, validatedData) {
			if (err) {
				...
				postCommentCollection.findOneAndUpdate(
					{ comment_id: comment_id, commented_by: commented_by },
					{ $set: validatedData },
					function (err, response) {
						if (err) {
							cb(err);
