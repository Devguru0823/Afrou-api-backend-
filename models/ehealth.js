// jshint esversion: 9
const ip = require('ip');
const Ehealth = require('./v2/Ehealth.model');
...
const FlutterWavePayment = require('./v2/FlutterWavePayment.model.js');
const { sendInvoiceEmail } = require('../_helpers/email-handler');
const { Testimonial } = require('./v2/Testimonials.model');
...
module.exports.getPlan = asyncHandler(async (req, res, next) => {
	...
	const query = {
		$or: [{ _id: id }, { 'subplans._id': id }],
	};

	const plan = await Ehealth.findById(id);
...
module.exports.getSingleUserPurchasedPlan = asyncHandler(
	async (req, res, next) => {
		...
		const history = await History.findById(id)
			.select('transaction plan createdAt')
			.populate('transaction');
