...
const { getUserPhotos } = require('./user_photos');
...
module.exports.usertofollow = function (req, res, cb) {
	utility.mongoConnect(req, res, function (CLIENT) {
		...
		userCollection
			.aggregate([
				{ $match: { registerFrom: 'autobot', status: 'active' } },
				{ $sample: { size: 1 } },
			])
			.toArray(async (err, autoRegUser) => {
				if (err) {
					...
					console.log(autoRegUser[0]);
					var currentUserProfile = autoRegUser[0];

					/**
					 * Get Active user in random order
					 * Active user = do any activity in last 30 days
					 */
					userCollection
						.aggregate([
							{
							...
							},
							{
								$match: {
									...
									status: 'active',
									user_id: { $ne: currentUserProfile.user_id },
								},
							},
							{ $sample: { size: 1 } },
						])
						.toArray(async (err, userList) => {
							if (err) {
								...
								if (
									...
									currentUserProfile.following_ids.indexOf(
										otherUserProfile.user_id
									) !== -1
								) {
									isFollowing = true;
								...
								// Check if Other user following
								let isOFollowing = false;
								if (
									otherUserProfile.following_ids &&
									otherUserProfile.following_ids.indexOf(
										currentUserProfile.user_id
									) !== -1
								) {
									isOFollowing = true;
								}
								// console.log("isOFollowing: " + isOFollowing);
										...
										userCollection.findOneAndUpdate(
											{
												...
												following_ids: { $ne: friend_id },
											},
											{ $push: { following_ids: friend_id } },
											{ returnOriginal: false },
											function (err, myProfileDetails) {
												if (err) {
													cb(err);
												} else {
													userCollection.findOneAndUpdate(
														{
															user_id: friend_id,
															follower_ids: { $ne: currentUserId },
														},
														{ $push: { follower_ids: currentUserId } },
														{ returnOriginal: false },
																		...
																		NOTIFICATION.addNotification(
																			...
																			function (err, notificationResp) {
																				...
																				if (myProfileDetails.value) {
																					finalResponse.data = {
																						friend_ids:
																							myProfileDetails.value.friend_ids,
																						following_ids:
				...
				if (err) {
					...
					userCollection
						.aggregate([
							{
								$match: {
									registerFrom: 'autobot',
									status: 'active',
									following_ids: { $nin: [currentUserProfile.user_id] },
								},
							},
							{ $sample: { size: 1 } },
						])
...
function getUserToFollow(curUser, userCollection, cb) {
	...
	userCollection
		.aggregate([
			...
			{
				$match: {
					last_active: { $lt: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) },
					status: 'active',
				},
			},
			{ $sample: { size: 1 } },
...
const sendFollowRequest = function (
	CLIENT,
	req,
	res,
	...
	let CONNECTION = CLIENT.db(utility.dbName);
	let followRequestCollection = CONNECTION.collection(
	...
	userCollection.countDocuments(
		{ user_id: requested_by, following_ids: requested_to },
		function (err, count) {
			if (count > 0) {
							...
							utility.validatePostData(
								...
								function (err, validatedData) {
									if (err) {
										...
										followRequestCollection.insertOne(
											validatedData,
											function (err, response) {
												if (err) {
													if (err.code === 11000) {
														let error = new Error(
															'You have already sent Follow Request'
														);
														cb(error);
													} else {
														cb(err);
													...
													userCollection
														.find({
															user_id: { $in: [requested_by, requested_to] },
														})
														.toArray(async (err, usersData) => {
															// Send Email
															let fromUser = null;
															...
															EMAIL_HANDLER.sendFollowRequestEmail(
																fromUser,
																toUser
															);
