    ...
    utility.mongoConnect(req, res, function (CLIENT) {
        ...
        userCollection.aggregate(
            ...
            if (err) {
                ...
                userCollection.aggregate(
                    [
                        { $project : { "user_id" : 1, "first_name":1, "last_name":1, "status":1, "last_active" : 1, "firebase_token": 1, "private": 1 } },
                        { $match: { "last_active":{$gt:new Date(Date.now() - 30*24*60*60*1000)}, "status": "active", "user_id":{ $ne: currentUserProfile.user_id } } },
                        { $sample: { size: 1 } } 
                    ]
        ...
        if (count > 0) {
            ...
            followRequestCollection.countDocuments({ 
                ...
                if (count > 0) {
                    ...
                    utility.validatePostData(CONNECTION, newFollowRequestData, FRIEND_REQUEST_MODEL, 'insert', 0, function (err, validatedData) {
                        if (err) {
                            cb(err);
                        } else {
