...
module.exports.getFollowingList = function (CLIENT, req, res, callback) {
	let CONNECTION = CLIENT.db(utility.dbName);
	let userCollection = CONNECTION.collection(USER_MODEL.collection_name);
	userCollection
		.aggregate([
 ...
 * @param callback
 */
module.exports.getFollowerList = function (CLIENT, req, res, callback) {
	let CONNECTION = CLIENT.db(utility.dbName);
	let userCollection = CONNECTION.collection(USER_MODEL.collection_name);
	...
	if (user_id === req.authorization.user_id) {
			...
			.toArray((err, userDataArr) => {
				...
				messages.getMessagesList(CLIENT, req, res, (err, chatList) => {
					...
					finalOutput.data.profile_strength =
						Math.round((profileSize / (totalSize * 2)) * 100) || 0;
					if (finalOutput.data.two_fa_secret) {
						delete finalOutput.data.two_fa_secret;
					}

					finalOutput.user_id = finalOutput.data.user_id;

					// Encrypt response
					const cipherUserDetails = CyptoJs.AES.encrypt(
						JSON.stringify(finalOutput.data),
		...
		userCollection
			.find(
				...
				{ projection: { password: 0, verification_token: 0 } }
			)
							...
							.toArray((err, requests) => {
								...
								userCollection
									...
									.toArray((err, extraFriendFollowingDetails) => {
										...
										if (otherUserProfile.two_fa_secret) {
											delete otherUserProfile.two_fa_secret;
										}
										// ecnrypt response
										const cipherUserDetails = CyptoJs.AES.encrypt(
