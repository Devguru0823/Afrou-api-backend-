...
const postProjection = {
  $project: {
    ...
    "requestedUser": { $cond: [{ $gt: [{ $arrayElemAt: ['$requestedUsers.count', 0] }, 0] }, true, false] },
    post_view_count: 1,
  }
};
      ...
      pipeline: [
        ...
        },
        {
          $project: {
            ...
            profile_image_url: 1,
            private: 1,
            following: { $cond: [{ $in: [currentUser, { $ifNull: ["$follower_ids", []] }] }, true, false] },
            enable_follow: { $cond: [{ $eq: [currentUser, "$user_id"] }, false, true] }
          }
        }
      ],
      as: "userDetails"
      ...
      pipeline: [
        ...
        getCommentLikesLookup(userId),
        {
          $project: {
            ...
            comment_text: 1,
            comment_date: 1,
            my_comment: { $cond: [{ $eq: ["$commented_by", userId] }, true, false] },
            "commented_by.first_name": { $arrayElemAt: ["$userDetails.first_name", 0] },
            "commented_by.last_name": { $arrayElemAt: ["$userDetails.last_name", 0] },
            "commented_by.profile_image_url": { $arrayElemAt: ["$userDetails.profile_image_url", 0] },
            "commented_by.user_id": { $arrayElemAt: ["$userDetails.user_id", 0] },
            "commented_by.user_name": { $arrayElemAt: ["$userDetails.user_name", 0] },
            "sub_comments": 1,
            "liked": { $cond: [{ $eq: [{ $arrayElemAt: ['$liked.count', 0] }, 1] }, true, false] },
            "comment_likes": { $cond: [{ $gt: [{ $arrayElemAt: ['$comment_likes_count.count', 0] }, 0] }, { $arrayElemAt: ['$comment_likes_count.count', 0] }, 0] },
          }
        }
      ],
      as: "comments"
...
module.exports.createStoryline = function (CLIENT, req, res, cb) {
  ...
  getAddressFromLatLong(newPostData.post_lat_long, async (locationName) => {
    ...
    // var newPostData2 = [];
    var counter = 0;
    if (newPostData.story_type && newPostData.story_type == 'image' && newPostData.story_image.length > 1) {
      for (var i = 0; i < newPostData.story_image.length; i++) {
        await getProperty(req.body, i, async function (newPostData2) {
          await utility.validatePostData(CONNECTION, newPostData2, Model, 'insert', 0, async function (err, validatedData) {
            if (err) {
              cb(err);
            } else {
              await storyCollection.insertOne(validatedData, async function (err, response) {
                if (err) {
                  cb(err);
                } else {
                  counter++;
                  if (i == counter) {
                    ...
                    const userCollection = CONNECTION.collection(userCollectionName);
      ...
      utility.validatePostData(CONNECTION, newPostData, Model, 'insert', 0, async function (err, validatedData) {
        if (err) {
          cb(err);
        } else {
          console.log(validatedData);
          storyCollection.insertOne(validatedData, async function (err, response) {
            if (err) {
              cb(err);
            } else {
              /* Get followers and send notification */
...
module.exports.updateStorylineView = function (CLIENT, req, res, cb) {
  ...
  storyCollection.findOneAndUpdate({ story_id: story_id }, { $addToSet: { seen_by: user_id } }, (err, updated) => {
    if (err) { cb(err); }
    ...
    storyCollection.findOneAndUpdate({ story_id: story_id }, { $set: { post_view_count: curView } }, (err, updated1) => {
      if (err) { cb(err); }
      let finalResponse = {};
      finalResponse.status = true;
...
const getAddressFromLatLong = (latLong, cb) => {
  ...
  request.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${latLong}&key=${SITE_CONFIG.googleAPIKey}`, (error, response, body) => {
    if (!error && response.statusCode == 200) {
      const info = JSON.parse(body);
      const locationArr = [];
      const countryIndex = info.results[0].address_components.findIndex(x => x.types.includes('country'));
      if (countryIndex !== -1) {
        locationArr.push(info.results[0].address_components[countryIndex].long_name);
      }

      const areaLevel1Index = info.results[0].address_components.findIndex(x => x.types.includes('administrative_area_level_1'));
      if (areaLevel1Index !== -1) {
        locationArr.unshift(info.results[0].address_components[areaLevel1Index].long_name);
      }

      const townIndex = info.results[0].address_components.findIndex(x => x.types.includes('postal_town'));
      if (townIndex !== -1) {
        locationArr.unshift(info.results[0].address_components[townIndex].long_name);
      } else {
        const areaLevel2Index = info.results[0].address_components.findIndex(x => x.types.includes('administrative_area_level_2'));
        if (areaLevel2Index !== -1) {
          locationArr.unshift(info.results[0].address_components[areaLevel2Index].long_name);
        }
      }
...
module.exports.editStoryline = function (CLIENT, req, res, cb) {
  ...
  utility.validatePostData(CONNECTION, body, Model, 'update', story_id, function (err, validatedData) {
    if (err) {
      cb(err);
    } else {
...
module.exports.deleteStoryline = function (CLIENT, req, res, cb) {
  ...
  storylineCollection.findOne({ story_id: story_id }, (err, postData) => {
    storylineCollection.deleteOne({ story_id: story_id, posted_by: req.authorization.user_id }, function (err, response) {
      if (err) {
        cb(err);
      } else {
        let finalResponse = {};
